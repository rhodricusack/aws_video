...
def run_video_rekognition(bucket,prefix,doevenifdone=False):
    ...
    for myobj in allfiles['Contents']:
        ...
        if myobj['Key'][-4:]=='.mp4':
            ...
            if doevenifdone or not 'Contents' in outputpresent:
    ...
    if doevenifdone or not 'Contents' in s3client.list_objects(Bucket=vid['S3Bucket'],Prefix=outkey_coding):
        try:
            ...
            if v._pth is None or not os.path.exists(v._pth):
                ...
                # Create summary dict
                summary={'coding':coding,'conf':conf,'possible_codes':possible_codes,'allfaces':allfaces,'behav':behav,'vid':vid,'compmsg':compmsg,'deltat':deltat,'fps':fps,'dur':dur}
...
def run_machine_learning(bucket,experimentfilters,outpth,colorscheme=None,possible_codes=None):
    """
    ...
    classifycols=['Pitch', 'Roll', 'Yaw', 'leftPupil', 'rightPupil','eyeLeft','eyeRight','EyesOpenValue','EyesOpenConfidence']
    ...
    for spind,sp in enumerate(experimentfilters):
        ...
        with open(fn,'rb') as f:
            ...
            for testsubjind,testdata in enumerate(obj):
                testdata['df_zerocentre']=testdata['df_dropna'][classifycols].subtract(testdata['df_dropna'][classifycols].mean())
            ...
            for testsubjind,testsubj in enumerate(obj):
                ...
                trainlabels= pd.concat([x[1]['df_dropna']['mancod'] for x in enumerate(obj) if not x[0] == testsubjind])
                ...
                # Signal detection theory trick - if hits or fa=0 then replace with half a trial, and same at max end
                ...
                mldf = mldf.append(pd.DataFrame({'proponeface':proponeface,
                                    ...
                                    'QualitySharpness':testsubj['df']['QualitySharpness'].mean(skipna=True),
                                    'QualityBrightness':testsubj['df']['QualityBrightness'].mean(skipna=True),
                ...
                ax=plt.subplot(111)
                bb = testsubj['df'][['BoundingBoxLeft', 'BoundingBoxTop', 'BoundingBoxWidth', 'BoundingBoxHeight']].mean()
                ax.add_patch(mpatches.Rectangle((bb[0],bb[1]-bb[3]),bb[2],bb[3],edgecolor=(1-proponeface,proponeface,0), Fill=False))
                # print(mldf.describe())
            ...
            plt.figure("proponeface")
            plt.hist(mldf['proponeface'], stacked=True,color=colorscheme[spind], bins=np.linspace(0,1,10))
#            sns.distplot(mldf['proponeface'],kde=False,rug=True,color=colorscheme[spind],bins=np.linspace(0,1,10))
    ...
    fig.savefig(os.path.join(outpth,'proponeface.pdf'),format='pdf')
...
def inter_rater_reliability(bucket,experimentfilter,mldf,pred,outpth,QualityBrightnessThreshold=None,color=None):
    ...
    for subjind,codobj in enumerate(s3bucket.objects.filter(Prefix=experimentfilter)):
        ...
        with open(fn, 'rb') as f:
            if QualityBrightnessThreshold is None or (mldf_thissubj['QualityBrightness']>=QualityBrightnessThreshold).any():
                obj = pickle.load(f)
                m = [[oneperson['code'] for oneperson in c['mancod_allraters'] if not oneperson['code'] is None] for c in obj['coding']]
                m=np.array(m)
                a=pred[subjind]
                ...
                df=df.append(pd.DataFrame({'man_man_kappa':np.mean(man_man_kappa),'man_auto_kappa':np.mean(man_auto_kappa)},index=[codobj.key]))

    if QualityBrightnessThreshold is None:
        nme="kappa"
    else:
        nme="kappa%f"%QualityBrightnessThreshold
    fig = plt.figure(nme)
    plt.subplot(211)

    plt.hist(df['man_man_kappa'].dropna(),stacked=True,color=color,bins=np.linspace(-1,1,20))
    plt.xlim([-1,1])
    plt.xlabel('')
    plt.ylabel('Human-Human')
    plt.subplot(212)
    plt.hist(df['man_auto_kappa'].dropna(),color=color,bins=np.linspace(-1,1,20))
    plt.xlim([-1,1])
    if QualityBrightnessThreshold is None:
        plt.xlabel('Inter-rater reliability (Cohen''s kappa)')
    else:
        plt.xlabel('Inter-rater reliability (Cohen''s kappa, QualityBrightness Threshold %f'%QualityBrightnessThreshold)
    plt.ylabel('Human-classifier')

    fig.savefig(os.path.join(outpth,'%s.pdf'%nme),format='pdf')
...
#                reprocess_video(bucket,experimentfilter)
            ...
            #for experimentfilter in experimentfilters:
            #    post_rekognition_summary(bucket,'coding/'+experimentfilter)

            # Run leave-one-out machine learning
            mlres=run_machine_learning(bucket,['coding_summary/coding/'+experimentfilter for experimentfilter in experimentfilters],
                                       outpth, possible_codes=[1,2],colorscheme=colorscheme)
...
#                reprocess_video(bucket,experimentfilter)
            ...
            mlres = run_machine_learning(bucket, ['coding_summary/coding/' + experimentfilter for experimentfilter in
                                                  experimentfilters], outpth, possible_codes=[-1,1],colorscheme=colorscheme)
        ...
        for ind, experimentfilter in enumerate(experimentfilters):
            irr[experimentfilter]=inter_rater_reliability(bucket, 'coding/' + experimentfilter, mlres['allmldf'], mlres['allpred'][ind],
                                          outpth,color=colorscheme[ind])
        # Make table of irr
        with open(os.path.join(outpth, 'irr.txt'), 'w') as f:
            print("Summarising IRR", f)
